...
variable "tfe_fqdn" {
  type        = string
  description = "Fully qualified domain name of TFE instance. This name should resolve to the load balancer DNS name or IP address and will be what clients use to access TFE."
}
...
variable "asg_instance_count" {
  type        = number
  description = "Desired number of TFE EC2 instances to run in Autoscaling Group. Leave as `1` unless`tfe_operational_mode` is `active-active`."
  default     = 1
}

variable "asg_max_size" {
  type        = number
  description = "Max number of TFE EC2 instances to run in Autoscaling Group."
  default     = 3
}
